...
void HttpConnection::Start() {
	CheckDeadline();					//开始时就启动超时保护，以防客户端建立连接但不发消息而产生冗余HttpConnection
	auto self = shared_from_this();				//获取当前HttpConnection对象的shared_ptr
	http::async_read(_socket, _buffer, _request,				
		[self](beast::error_code ec, std::size_t bytes_transferred) {		//bytes_transferred可以控制一次read操作从socket中读取多少字节
		try {
			if (ec) {
			...
			boost::ignore_unused(bytes_transferred);		//http有消息边界，故不需要处理粘包，忽视参数
			self->HandleReq();
		}
